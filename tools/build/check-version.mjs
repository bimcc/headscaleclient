...
const wailsVersion = read('go.mod').match(/github\.com\/wailsapp\/wails\/v3 v([^\s]+)/)[1];
check('Wails frontend runtime', JSON.parse(read('frontend/package.json')).dependencies['@wailsio/runtime'], wailsVersion);
check('Wails CI CLI', read('.github/workflows/ci.yml').match(/WAILS_VERSION:\s*"v([^"]+)"/)[1], wailsVersion);
check('Wails application diagnostics', read('main.go').match(/WithDiagnostics\(appVersion, "([^"]+)"/)[1], wailsVersion);
check('Wails service diagnostics', read('internal/application/service.go').match(/wailsVersion:\s*"([^"]+)"/)[1], wailsVersion);
check('Wails demo diagnostics', read('frontend/src/lib/backend.ts').match(/wailsVersion:\s*"([^"]+)"/)[1], wailsVersion);
const windows = JSON.parse(read('build/windows/info.json'));
check('Windows file version', windows.fixed.file_version);
check('Windows fixed product version', windows.fixed.product_version);
check('Windows display file version', windows.info['0409'].FileVersion);
check('Windows display product version', windows.info['0409'].ProductVersion);
